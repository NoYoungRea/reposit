...
## 파일 IO
2. 동시에 같은 파일을 여러번 열지 않는 것이 좋다. 어떻게 될지 예측할 수 없다.

## fgets
- fgets: 버퍼보다 작은 값이 들어오면 개행이 포함되고 그 끝은 널로 끝납니다. 버퍼의 크기보다 같거나 큰 값이 들어오면 끝에 널을 추가합니다. 예를 들어 버퍼 크기가 32개인데 31개를 넣으면 개행이랑 널 들어가야 하기때문에 하나 빠진다. 결국 30개 들어간다.
