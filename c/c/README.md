...
- 배열을 다른 곳에서 보내서 참조하는 경우 반드시 행의 정보도 같이 보내야 한다.
...
### inline과 메크로
- 일반 함수의 선언문(정의문일듯?)은 헤더에 있으면 안되지만(컴파일 타임엔 ㄱㅊ but 링크시 에러) 인라인은 컴파일 하는 단계에서 기계어로 박아주는 것이기 때문에 ㄱㅊ 메크로 또한 전처리기 단계에서 해당 기능을 치환해주는 개념이기에 ㄱㅊ
...
## 입력(0552-7)
- 프로그래밍이 실행되면 두개의 장치가 붙는다. (키보드 모니터) 이런것들을 프로그램은 다 파일로 본다. 키보드도 더이상 보낼 것이 없다면 eof를 보낸다.
...
## 입력 범위
- gets 쓰면 범위를 넘어 갔을때 오류가 난다.	
- fgets을 쓰면 범위를 넘어가면 알아서 잘라준다. <-이것 사용하자!!! 
- scanf를 통해서 길이제한 주고 싶다면 %뒤에 길이 제한 두면 된다.(포맷 지정자)

## 구조체 에서 arr[0] 사용
1. 구조체에서 정적 배열 사용하면 낭비. so 포인터 사용->할당을 해 줘야 하고 자원해제를 두번에 걸쳐 다 해줘야 하는 번거로움이 있다.(성능상의 이슈와 (2번할당) 누수 가능성이 있다.)
2. arr[0]이나 arr[1]을 사용하여 arr를 참조할 수 있게만 하고 실제 할당은 더 크게 한다.
3. flexible array member(C99): 배열의 길이 정보가 없는 멤버 필드
- 제약 조건: 반드시 구조체의 마지막 멤버로 위치해야 합니다.
- 반드시 1개만 존재해야 합니다.
- arr[0]은 컴파일러마다 안 되는 곳이 있다. arr[]가 표준이니 이 것을 사용

## 파일 IO 
1. 동시에 같은 파일을 여러번 열지 않는 것이 좋다. 어떻게 될지 예측할 수 없다.
2. 입출력 파일 객체에 대해서 복사본을 사용하면 안 된다.

## fgets
- 버퍼보다 작은 값이 들어오면 개행이 포함되고 그 끝은 널로 끝납니다. 버퍼의 크기보다 같거나 큰 값이 들어오면 끝에 널을 추가합니다. 예를 들어 버퍼 크기가 32개인데 31개를 넣으면 개행이랑 널 들어가야 하기때문에 하나 빠진다. 결국 30개 들어간다.
- 무조건 적으로 개행 삭제하면 안 되고 검사를 한 번 해야 한다. (ex)strchr
- 실패하면 원본 버퍼는 알 수 없는 상태로 정의되기 때문에 사용하면 안 된다. 그러므로 실패시 해당 버퍼를 널 문자열로 설정해 놓아라

## C에서 두번 해제 하면 안 되므로
- 널 주소를 해제하는 것은 괜찮기때문에 포인터를 해제한 후 무조건 null을 참조해주자. 이렇게 해 놓으면 나중에 두번 해제하더라도 괜찮다. 차라리 그냥 라이브러리로 만들어 놓는 것도 좋다.

## error&perror
1. error를 사용하려면errno.h 헤더를 포함하자.
2. 과정이 stderr출력스트림을 통해 'fopen error!!'(사용자 지정): 3번 이런식으로 찍고 싶은데 에러가 났을때 errno값이 알아서 지정되므로 가져와서 찍기만 하면 된다. but 번호는 알아듣기 어렵기 때문에 strerror을 통해 번호를 string으로 바꿔준다. 그리고 이걸 함수로 빼서 인자로는 사용자 지정 메세지만 받아서 출력 하는 메서드를 만들자. 사실 이거 라이브러리로 존재함 -> perror


## stderr
- 프로그램이 실행되면 운영체제가 stdin stdout stderr 3개를 붙여준다.!!!!!!!!!! stderr은 버퍼가 없다.
- stderr 로 출력하게 되면 바로 바로 출려됨(버퍼에 쌓였다가 한번에 출력되는 것이 아님 에초에 버퍼가 없음) 실험적으로 hellow 출력하고 exit 하면 hellow 출력 안 되는데 stderr로 하면 됨.(exit vs _exit 차이점 flush 하고 안 하고 차이)

## strtol
- 범위를 넘어간 값이 나오면 longMax나 longMin이 리턴된다. 진또배기 longMax,Min을 거르려면 errno=ERANGE인지 확인해주어야 한다. endPtr이 시작주소에 멈춰있는지도 확인하자



## asmably
- 외부의 빌더를 이용하고 싶을때 항목형식 사용자 지정 빌드 도구
- nasm -f win32 -o first.obj first.asm -> nasm을 이용하여 win32 first.asm을 이용하여 first.obj를 만들겠다.
- 저거는 vs의컴파일러를 이용하여 목적파일을 만들고 저 어셈블리는 우리가 다운받은 어셈블러를 이용하여 목적 파일을 만들어 링커로 연결
- 함수가 리턴할때 eax라는 레지스터를 사용
- 복귀 주소나 인수의 경우 register을 이용해도 되지만 크기가 제한되어 있으니 stack을 사용하자.
- 어셈블리에서 함수를 호출할때 다시 돌아가야 하기 때문에 복귀 주소를 계속 stack에 쌓아 나가고 복귀할때는 그것을 꺼내서 복귀한다. 이것을 push 와 pop을 명시적으로 써줘야 하는 것을 한 줄로 줄이는 것이 'call 함수'와 ret(복귀주소를 pop으로 꺼내어 jmp로 해당주소로 이동)을 통해 return하는 것이다.
- 인수의 경우에는 push를 통해 인수를 stack에 저장하고 esp(stack의 top의 역활을 함)의 주소로 부터 4배수 단위로 참조하여 인수의 값을 가져올 수 있다. 중요한 것은 인수 메모리를 파괴하는 것인데 
1. __cdecl: 호출자가 해제 하는 방식 -> C 표준
- cdecl은 C DECLaration의 약자입니다.
- 가변 인자를 구현할 수 있다는 장점이 있습니다. 함수를 호출한 쪽에서 인자를 전달한 개수를 알 수 있기 때문입니다.
- ex)int printf(const char *fmt, ...);
- 단점은 함수 호출이 끝날 때마다 스택을 해제하는 코드가 추가되므로 프로그램의 크기가 커질 수 있습니다.
- 구현)호출 시킨 곳에서 함수에 갔다 온 후 esp의 위치를 8만큼 더하여(위로 이동, 인수 두개 파괴) 파괴하는 방법

2. __stdcall: 피호출자가 해제 하는 방식 -> Win32 API 등
- 스택을 해제하는 코드가 함수 안에 있으므로 프로그램의 크기가 상대적으로 작아진다는
장점이 있습니다.
- 단점은 가변 인자 함수를 구현할 수 없습니다.
- 구현)이것은 ret 8이런식으로 esp위치를 8만큼 더한 다음 리턴함으로 파괴한다.
...
6. 자료형의 크기가 플랫폼다 다를 수 있으므로 그 전에 확인하자.
7. 이항연산자는 양쪽의 타입이 일치하지 않으면 계산하지 않음. 근데 해보면 왜 돼? 작은 타입을 큰 타입으로 암시적 형변환을 해 준다.
8. char로 해줘도 어차피 4바이트로 잡힘. 웬만하면 int 쓰셈. 가장 빠름.
9. 문자열 토큰 함수 strtok 쓸때 원본도 잘린다는 것 인지하기 바람.
10. 사용자 지정 FREE 함수 만들어 놓는 것이 좋다.(구현부에서는 해제해준 후에 null을 가리키도록 주소를 바꾸어 놓자. 그리고 해제하기 전에 그 자리에 있던 데이터들을 다 clean 해 놓는 것이 좋다.) 단, 2~3줄을 저렇게 함수로 빼는 것은 함수 오버헤드이므로 메크로를 사용하자.
11. malloc 시리즈 할때 반환 형을 오른쪽으로 명시적으로 해주는 것이 좋다. 그래야ㅐ 컴파일 타임에 오류나 경고를 발생시킬 수 있다. 
12. 배열에 변수가 올 수 있다. (단 c99 에서 만)
13. 에러의 경우 사용자에게 메세지를 보내 사용자에게 처리하는 것이 좋다.
14. c 언어는 mangling(for overloading)하지 않는다.
15. 스택은 높은 주소에서 낮은 주소로 쌓아가고 heap은 낮은 주소에서 높은 주소로 4바이트씩 증가 
16. 문자열을 char 포인터에 담을때 const를 써주는 이유 : 컴파일때 에러내기 위해서
17. "문자열" 이것도 연산자다. 이렇게 하면, 아스키 널을 끝에 넣어준다.
